...
关于GUI环境
运行install.sh之后，GUI模式对于droplet.py来说立即生效，无需其他设置。
如果此时想在命令行手动运行GUI模式的脚本，那么当前ssh里缺少一个必要的环境变量。
重新连接ssh可解决。或执行以下语句立即解决：
export DISPLAY=:99
...
python3 droplet.py [-n 服务器名称] [-t 服务器类型] [其他可选参数]
------------------------------------------------------------------------
...
其他参数：
    ...
    -ap 专门针对login类型droplet。它指明开启login脚本时如何分配代理。可接两个附加参数group和tag。不带次参数则不使用代理。
...
***************************
cookiemaker.py命令行参数用法
***************************


python3 cookiemaker.py [-c] [-x 代理地址:端口:用户名:密码]
------------------------------------------------------------------------
默认情况打开firefox。如果指定了-c（--chrome），则打开chrome。
支持使用-x（--proxy）参数初始化浏览器代理。
打开浏览器后，手动进行任何操作。完成操作之后，回到命令行界面，输入"r"，关闭浏览器。
此时，脚本已经保存本次浏览器会话中产生的cookies。
按照脚本接下来的提示，输入instagram用户名和密码，脚本会在主服务器为该instagram创建一条数据。
该数据包含相应的用户名，密码和cookies，以供run.py脚本pull使用。
...
【举例1】python3 run.py minhaodeng -pe proxy -ap
pull最近成功登陆的信息，排除代理。然后用-ap获取新代理。
【举例2】python3 run.py minhaodeng -pe tasks -rt follow-800
pull上次的参数，但排除本地tasks，这次使用远程任务follow-800


-pb, --pull-by [group1] [group2] ... （此参数自1.50版本起失效）
------------------------------------------------------------------------
...
-c, --chrome
------------------------------------------------------------------------
使用chrome。此参数缺省时默认用firefox。
chrome仅在gui模式下支持代理。所以在使用chrome时如果同时使用代理，需要再额外指定-g（--gui）参数。
【举例】python3 run.py minhaodeng -p -px 代理地址:端口:用户名:密码 -g -c


-g, --gui
------------------------------------------------------------------------
使用gui模式。此参数缺省时默认为headless模式。
在无屏幕的ubuntu云服务器环境中，gui模式同样可用。
此时gui模式通过运行Xvfb服务来实现。display的端口被设置为99。
与Xvfb相关的环境在install.sh安装脚本中会自动配置完成。
