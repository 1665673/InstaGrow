...
------------------------------------------------------------------------
只需填写instagram用户名。然后自动从后台获取最新登录成功的密码，代理。
pull后面可以选跟若干个group参数，如果指定group，则pull按照group分组获得信息。
【举例】python3 like.py IG用户名 --pull version
那不同version的follow，like等脚本具有不同分组，在pull时会对应独立的信息。



-t, --tasks task1 [task2] ...
------------------------------------------------------------------------
指定要跑的tasks。目前仅run.py脚本支持。
